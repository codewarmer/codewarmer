...
(function(){function Q(a){function h(d){var a=d.charCodeAt(0);if(92!==a)return a;var f=d.charAt(1);return(a=u[f])?a:"0"<=f&&"7">=f?parseInt(d.substring(1),8):"u"===f||"x"===f?parseInt(d.substring(2),16):d.charCodeAt(1)}function k(d){if(32>d)return(16>d?"\\x0":"\\x")+d.toString(16);d=String.fromCharCode(d);return"\\"===d||"-"===d||"]"===d||"^"===d?"\\"+d:d}function l(d){var a=d.substring(1,d.length-1).match(/\\u[0-9A-Fa-f]{4}|\\x[0-9A-Fa-f]{2}|\\[0-3][0-7]{0,2}|\\[0-7]{1,2}|\\[\s\S]|-|[^-\\]/g);d=
[];var f="^"===a[0],b=["["];f&&b.push("^");for(var f=f?1:0,c=a.length;f<c;++f){var g=a[f];if(/\\[bdsw]/i.test(g))b.push(g);else{var g=h(g),e;f+2<c&&"-"===a[f+1]?(e=h(a[f+2]),f+=2):e=g;d.push([g,e]);65>e||122<g||(65>e||90<g||d.push([Math.max(65,g)|32,Math.min(e,90)|32]),97>e||122<g||d.push([Math.max(97,g)&-33,Math.min(e,122)&-33]))}}d.sort(function(d,c){return d[0]-c[0]||c[1]-d[1]});a=[];c=[];for(f=0;f<d.length;++f)g=d[f],g[0]<=c[1]+1?c[1]=Math.max(c[1],g[1]):a.push(c=g);for(f=0;f<a.length;++f)g=a[f],
...
l=[],q=0;q<k.length;++q)for(var s=0,v=k[q].length;s<v;++s)l.push(k[q][s]);var k=null,m=Date;m.now||(m={now:function(){return+new Date}});var n=0,b,p=/\blang(?:uage)?-([\w.]+)(?!\S)/,u=/\bprettyprint\b/,t=/\bprettyprinted\b/,d=/pre|xmp/i,w=/^code$/i,f=/^(?:pre|code|xmp)$/i;h()}};"function"===typeof define&&define.amd&&define(function(){return U})})();PR.registerLangHandler(PR.createSimpleLexer([[PR.PR_PLAIN,/^[ \t\r\n\f]+/,null," \t\r\n\f"]],[[PR.PR_STRING,/^\"(?:[^\n\r\f\\\"]|\\(?:\r\n?|\n|\f)|\\[\s\S])*\"/,null],[PR.PR_STRING,/^\'(?:[^\n\r\f\\\']|\\(?:\r\n?|\n|\f)|\\[\s\S])*\'/,null],["lang-css-str",/^url\(([^\)\"\']+)\)/i],[PR.PR_KEYWORD,/^(?:url|rgb|\!important|@import|@page|@media|@charset|inherit)(?=[^\-\w]|$)/i,null],["lang-css-kw",/^(-?(?:[_a-z]|(?:\\[0-9a-f]+ ?))(?:[_a-z0-9\-]|\\(?:\\[0-9a-f]+ ?))*)\s*:/i],[PR.PR_COMMENT,/^\/\*[^*]*\*+(?:[^\/*][^*]*\*+)*\//],
